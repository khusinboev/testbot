#!/usr/bin/env python3
"""
scripts/manage.py — Barcha baza amallari bitta joyda

Ishlatish:
    python scripts/manage.py <buyruq>

Buyruqlar:
    init        — Jadvallar + asosiy seed (fanlar, viloyatlar, yo'nalishlar)
    reset       — Bazani o'chirb qayta yaratish (EHTIYOT BO'LING!)
    seed        — Faqat savol seedlash (init dan keyin)
    migrate     — Mavjud bazaga yangi ustunlar qo'shish (is_archived, attempted_count)
    status      — Baza holatini ko'rish

Misol:
    python scripts/manage.py init        # yangi baza
    python scripts/manage.py migrate     # mavjud bazani yangilash
    python scripts/manage.py seed        # savollarni qo'shish
    python scripts/manage.py reset       # hammasini o'chib qayta boshlash
"""
import sys
import os
import random
...
def cmd_init():
    """Jadvallar yaratish + asosiy ma'lumotlar (fanlar, viloyatlar, yo'nalishlar)."""
    from database.db import init_db
    print("🚀 Baza initializatsiyasi boshlandi...")
    init_db()
    print("✅ Baza tayyor! Savollar uchun: python scripts/manage.py seed")


def cmd_reset():
    """Barcha jadvallarni o'chirib qayta yaratish."""
    confirm = input("⚠️  BARCHA MA'LUMOTLAR O'CHADI! Davom etish uchun 'yes' yozing: ").strip()
    if confirm != "yes":
        print("Bekor qilindi.")
        return
    ...
    drop_tables()
    print("🔧 Jadvallar qayta yaratilmoqda...")
    init_db()
    print("✅ Baza qayta yaratildi!")


# ─────────────────────────────────────────────────────────────────────────────
# MIGRATE — mavjud bazaga yangi ustunlar
# ─────────────────────────────────────────────────────────────────────────────

def cmd_migrate():
    """Mavjud bazaga yangi ustunlar qo'shish (ma'lumotlarni saqlab)."""
    ...
    steps = [
        ...
        ("scores.total_questions = 90",
         "UPDATE scores SET total_questions = 90 WHERE total_questions != 90"),
        ...
        ("user_test_participation.deadline_at",
         "ALTER TABLE user_test_participation ADD COLUMN IF NOT EXISTS deadline_at TIMESTAMP"),
        ("user_test_participation.snapshot_questions",
         "ALTER TABLE user_test_participation ADD COLUMN IF NOT EXISTS snapshot_questions JSON"),
        ("user_test_participation.snapshot_current_index",
         "ALTER TABLE user_test_participation ADD COLUMN IF NOT EXISTS snapshot_current_index INTEGER DEFAULT 0"),
        ("user_test_participation.snapshot_answers",
         "ALTER TABLE user_test_participation ADD COLUMN IF NOT EXISTS snapshot_answers JSON"),
        ("scores.participation_id",
         "ALTER TABLE scores ADD COLUMN IF NOT EXISTS participation_id INTEGER REFERENCES user_test_participation(id)"),
        ("users.is_blocked",
...
CREATE INDEX IF NOT EXISTS idx_referral_invites_invited_user ON referral_invites(invited_user_id);
    ...
    try:
        ...
        from database.models import MandatoryChannel, BroadcastMessage
        Base.metadata.create_all(bind=engine, tables=[
            MandatoryChannel.__table__,
            BroadcastMessage.__table__,
        ])
        print("✅ mandatory_channels, broadcast_messages jadvallari (yangi yoki mavjud)")
    except Exception as e:
        print(f"⚠️  Jadvallar yaratish: {e}")
...
QUESTIONS_DATA = {
    ...
    2: [  # Fizika
        ...
        {"text_uz": "Mexanik to'lqin tarqalishi uchun nima kerak?", "option_a": "Vakuum", "option_b": "Modda", "option_c": "Magnit maydon", "option_d": "Yorug'lik", "correct_answer": "B"},
        ...
        {"text_uz": "Diffraktsiya nima?", "option_a": "To'lqin sinishi", "option_b": "To'lqin to'siqlar atrofida egilishi", "option_c": "To'lqin qaytishi", "option_d": "To'lqin kuchayishi", "correct_answer": "B"},
        ...
        {"text_uz": "Radioaktivlik qaysi hodisaga misol?", "option_a": "Yorug'lik sinishi", "option_b": "Elektroliz", "option_c": "Yadro reaksiyasi", "option_d": "Magnit induksiyasi", "correct_answer": "C"},
    ...
    3: [  # Kimyo
        ...
        {"text_uz": "NaOH dissotsilanishida nima hosil?", "option_a": "H⁺ ionlar", "option_b": "Na⁺ va OH⁻", "option_c": "O²⁻", "option_d": "Na va O", "correct_answer": "B"},
        {"text_uz": "Organik kimyo asosi qaysi element?", "option_a": "Azot", "option_b": "Kislorod", "option_c": "Uglerod", "option_d": "Vodorod", "correct_answer": "C"},
        ...
        {"text_uz": "Vodorod atomi nechtа proton?", "option_a": "0", "option_b": "1", "option_c": "2", "option_d": "3", "correct_answer": "B"},
        ...
        {"text_uz": "Kislota+asos reaksiyasi nima?", "option_a": "Oksidlanish", "option_b": "Neytrallash", "option_c": "Parchalanish", "option_d": "Birikish", "correct_answer": "B"},
        ...
        {"text_uz": "Kislota va asosning reaksiyasi mahsuloti?", "option_a": "Tuz va suv", "option_b": "Oksid va suv", "option_c": "Kislota va asos", "option_d": "Faqat tuz", "correct_answer": "A"},
    ...
    4: [  # Biologiya
        {"text_uz": "Fotosintez qaysi organoidda?", "option_a": "Yadro", "option_b": "Mitoxondriya", "option_c": "Xloroplast", "option_d": "Lizosoma", "correct_answer": "C"},
        ...
        {"text_uz": "Odamda nechta juft xromosoma?", "option_a": "21", "option_b": "22", "option_c": "23", "option_d": "24", "correct_answer": "C"},
        ...
        {"text_uz": "Inson yuragida nechta kamera?", "option_a": "2", "option_b": "3", "option_c": "4", "option_d": "5", "correct_answer": "C"},
        {"text_uz": "Kraxmalni parchalovchi ferment?", "option_a": "Pepsin", "option_b": "Lipaza", "option_c": "Amilaza", "option_d": "Tripsin", "correct_answer": "C"},
        ...
        {"text_uz": "Siydik qaysi organda hosil?", "option_a": "Jigar", "option_b": "Taloq", "option_c": "Buyrak", "option_d": "O't pufagi", "correct_answer": "C"},
        {"text_uz": "D vitamini nimani ta'minlaydi?", "option_a": "Temir so'rilishi", "option_b": "Kalsiy so'rilishi", "option_c": "Yod so'rilishi", "option_d": "Magniy so'rilishi", "correct_answer": "B"},
        {"text_uz": "Meyoz natijasida nechta hujayra?", "option_a": "2", "option_b": "4", "option_c": "8", "option_d": "16", "correct_answer": "B"},
        ...
        {"text_uz": "O'simlik hujayrasi + hayvon hujayrasida yo'q organoid?", "option_a": "Mitoxondriya", "option_b": "Ribosoma", "option_c": "Xloroplast", "option_d": "Yadro", "correct_answer": "C"},
        ...
        {"text_uz": "Ovqat hazm qayerdan boshlanadi?", "option_a": "Oshqozon", "option_b": "O'n ikki barmoqli ichak", "option_c": "Og'iz bo'shlig'i", "option_d": "Qizil o'ngach", "correct_answer": "C"},
        ...
        {"text_uz": "Insulin qaysi organ?", "option_a": "Jigar", "option_b": "Buyrak", "option_c": "Oshqozon osti bezi", "option_d": "Taloq", "correct_answer": "C"},
        {"text_uz": "Ko'z to'r pardasi hujayralar?", "option_a": "Tayoqcha va kolba", "option_b": "Neyron va glia", "option_c": "Epiteliy va muskul", "option_d": "Pigment va shaffof", "correct_answer": "A"},
        {"text_uz": "Odamda juft qovurg'alar soni?", "option_a": "10", "option_b": "12", "option_c": "14", "option_d": "16", "correct_answer": "B"},
        ...
        {"text_uz": "Sut emizuvchilarda miya eng rivojlangan?", "option_a": "Ha", "option_b": "Yo'q", "option_c": "Baliqlarda", "option_d": "Hasharotlarda", "correct_answer": "A"},
        {"text_uz": "O'simlik suv-tuz o'zlashtirishi qayerdan?", "option_a": "Barg", "option_b": "Poya", "option_c": "Ildiz", "option_d": "Gul", "correct_answer": "C"},
    ],
    5: [  # Tarix
        {"text_uz": "O'zbekiston qachon mustaqil?", "option_a": "1990", "option_b": "1991", "option_c": "1992", "option_d": "1993", "correct_answer": "B"},
        {"text_uz": "Amir Temur vafoti?", "option_a": "1404", "option_b": "1405", "option_c": "1406", "option_d": "1407", "correct_answer": "B"},
        {"text_uz": "1-Jahon urushi boshlanishi?", "option_a": "1912", "option_b": "1913", "option_c": "1914", "option_d": "1915", "correct_answer": "C"},
        ...
        {"text_uz": "Buyuk Ipak yo'li shakllangan?", "option_a": "M.av. I asr", "option_b": "M.av. II asr", "option_c": "Milodiy I asr", "option_d": "Milodiy II asr", "correct_answer": "B"},
        ...
        {"text_uz": "Ibn Sino tibbiyotni tizimlashtirgan asar?", "option_a": "Kitob al-Qonun", "option_b": "Avesto", "option_c": "Temurnoma", "option_d": "Boburnoma", "correct_answer": "A"},
        {"text_uz": "Fransiya inqilobi?", "option_a": "1776", "option_b": "1789", "option_c": "1812", "option_d": "1848", "correct_answer": "B"},
        {"text_uz": "Amerika mustaqillik deklaratsiyasi?", "option_a": "1775", "option_b": "1776", "option_c": "1777", "option_d": "1778", "correct_answer": "B"},
        ...
        {"text_uz": "Ulug'bek rasadxonasi qurilishi?", "option_a": "1420s", "option_b": "1450s", "option_c": "1400s", "option_d": "1380s", "correct_answer": "A"},
        {"text_uz": "G'arbiy Rim imperiyasi qulab tushishi?", "option_a": "375", "option_b": "410", "option_c": "476", "option_d": "527", "correct_answer": "C"},
        {"text_uz": "Temuriylar sulolasi barham topishi?", "option_a": "1500", "option_b": "1507", "option_c": "1510", "option_d": "1526", "correct_answer": "B"},
        {"text_uz": "Bobur Hindistonda qaysi sulolaga asos soldi?", "option_a": "Temuriylar", "option_b": "Safaviylar", "option_c": "Boburiylar", "option_d": "Shayboniylar", "correct_answer": "C"},
        {"text_uz": "O'zbekiston SSR tashkil topishi?", "option_a": "1922", "option_b": "1924", "option_c": "1925", "option_d": "1936", "correct_answer": "B"},
        ...
        {"text_uz": "Sovuq urush boshlanishi?", "option_a": "1944", "option_b": "1945", "option_c": "1947", "option_d": "1950", "correct_answer": "C"},
        {"text_uz": "MDH tashkil topishi?", "option_a": "1990", "option_b": "1991", "option_c": "1992", "option_d": "1993", "correct_answer": "B"},
        ...
        {"text_uz": "Ulug'bek kim tomonidan o'ldirildi?", "option_a": "Ko'chmanchilar", "option_b": "O'g'li Abdullatif", "option_c": "Dushman sarkardasi", "option_d": "Saroy fitnachilar", "correct_answer": "B"},
        {"text_uz": "Samarqand Temur poytaxtiga aylangan yil?", "option_a": "1370", "option_b": "1380", "option_c": "1390", "option_d": "1400", "correct_answer": "A"},
        {"text_uz": "Xiva xonligi tashkil topishi?", "option_a": "XVI asr boshi", "option_b": "XV asr oxiri", "option_c": "XVII asr boshi", "option_d": "XVIII asr", "correct_answer": "A"},
        {"text_uz": "Qo'qon xonligi tuzilishi?", "option_a": "XVII asr", "option_b": "XVIII asr boshi", "option_c": "XVIII asr o'rtasi", "option_d": "XIX asr boshi", "correct_answer": "B"},
        {"text_uz": "Rossiya O'rta Osiyoni to'liq bosib olish?", "option_a": "1868", "option_b": "1876", "option_c": "1885", "option_d": "1895", "correct_answer": "C"},
        {"text_uz": "Jadidchilik O'zbekistonda paydo bo'lishi?", "option_a": "XIX asr boshi", "option_b": "XIX asr o'rtasi", "option_c": "XIX asr oxiri", "option_d": "XX asr boshi", "correct_answer": "C"},
        {"text_uz": "O'rta asr Yevropa tarixi davri?", "option_a": "V-XV asrlar", "option_b": "I-V asrlar", "option_c": "XV-XVIII asrlar", "option_d": "III-VIII asrlar", "correct_answer": "A"},
    ],
    6: [  # Ona tili
        ...
        {"text_uz": "O'zbek lotin alifbosida nechta harf?", "option_a": "26", "option_b": "28", "option_c": "29", "option_d": "32", "correct_answer": "C"},
        ...
        {"text_uz": "Antonim so'zlar?", "option_a": "Ma'nodosh so'zlar", "option_b": "Qarama-qarshi ma'noli so'zlar", "option_c": "Ko'p ma'noli so'zlar", "option_d": "Shakldosh so'zlar", "correct_answer": "B"},
        {"text_uz": "Sinonim so'zlar?", "option_a": "Qarama-qarshi ma'noli", "option_b": "Ma'nodosh so'zlar", "option_c": "Shakldosh so'zlar", "option_d": "Ko'p ma'noli so'zlar", "correct_answer": "B"},
        ...
        {"text_uz": "O'zbek tilida unli tovushlar soni?", "option_a": "5", "option_b": "6", "option_c": "7", "option_d": "8", "correct_answer": "B"},
        ...
        {"text_uz": "'Kitoblar' so'zida nechta morfema?", "option_a": "1", "option_b": "2", "option_c": "3", "option_d": "4", "correct_answer": "B"},
    ...
    7: [  # Adabiyot
        ...
        {"text_uz": "'Layli va Majnun' dostonini kim yozgan?", "option_a": "Navoiy", "option_b": "Qodiriy", "option_c": "Cho'lpon", "option_d": "Hamza", "correct_answer": "A"},
        {"text_uz": "Navoiyning taxallusi?", "option_a": "Nizomiy", "option_b": "Husayn Boyqaro", "option_c": "Foniy", "option_d": "Bedil", "correct_answer": "C"},
        ...
        {"text_uz": "'Farhod va Shirin' kim?", "option_a": "Nizomiy", "option_b": "Navoiy", "option_c": "Fuzuliy", "option_d": "Bedil", "correct_answer": "B"},
        {"text_uz": "G'azal janri?", "option_a": "Epik", "option_b": "Dramatik", "option_c": "Lirik", "option_d": "Lirik-epik", "correct_answer": "C"},
        {"text_uz": "Ruboiy misralari soni?", "option_a": "2", "option_b": "3", "option_c": "4", "option_d": "6", "correct_answer": "C"},
        {"text_uz": "Doston janri?", "option_a": "Lirik", "option_b": "Dramatik", "option_c": "Epik yoki lirik-epik", "option_d": "Satirik", "correct_answer": "C"},
        {"text_uz": "Navoiyning ona tili?", "option_a": "Fors", "option_b": "Arab", "option_c": "Chig'atoy o'zbek", "option_d": "Mo'g'ul", "correct_answer": "C"},
        {"text_uz": "'Alpomish' — qaysi xalq eposi?", "option_a": "Qozoq", "option_b": "Qirg'iz", "option_c": "O'zbek", "option_d": "Turkman", "correct_answer": "C"},
        ...
        {"text_uz": "Tashbeh nima?", "option_a": "Ko'chma ma'no", "option_b": "Bir narsani boshqasiga o'xshatish", "option_c": "Qarama-qarshilik", "option_d": "Takror", "correct_answer": "B"},
        ...
        {"text_uz": "Shahnoma eposi qaysi millat?", "option_a": "O'zbek", "option_b": "Arab", "option_c": "Fors-tojik", "option_d": "Turk", "correct_answer": "C"},
        {"text_uz": "Abdulla Oripov mashhur she'rlar to'plami?", "option_a": "Qo'shiqlar", "option_b": "Munojot", "option_c": "O'zbegim", "option_d": "Bahor", "correct_answer": "C"},
        {"text_uz": "Erkin Vohidov mashhur janri?", "option_a": "Proza", "option_b": "Drama", "option_c": "She'riyat", "option_d": "Publitsistika", "correct_answer": "C"},
        {"text_uz": "Hajviy asar nima?", "option_a": "Qayg'uli", "option_b": "Keskin tanqidiy yoki kulgili", "option_c": "Qahramonlik", "option_d": "Romantik", "correct_answer": "B"},
        ...
        {"text_uz": "Dostondagi qahramonlar sevgisi qaysi janr?", "option_a": "Epik", "option_b": "Lirik", "option_c": "Lirik-epik", "option_d": "Dramatik", "correct_answer": "C"},
    ...
    8: [  # Geografiya
        {"text_uz": "O'zbekiston poytaxti?", "option_a": "Samarqand", "option_b": "Buxoro", "option_c": "Toshkent", "option_d": "Andijon", "correct_answer": "C"},
        ...
        {"text_uz": "Yerning eng baland nuqtasi?", "option_a": "K2", "option_b": "Kanchenjunga", "option_c": "Everest", "option_d": "Lxotze", "correct_answer": "C"},
        ...
        {"text_uz": "Eng ko'p aholiga ega davlat?", "option_a": "Hindiston", "option_b": "Xitoy", "option_c": "AQSH", "option_d": "Rossiya", "correct_answer": "A"},
        {"text_uz": "Yer o'qi atrofida aylanish muddati?", "option_a": "12 soat", "option_b": "24 soat", "option_c": "365 kun", "option_d": "30 kun", "correct_answer": "B"},
        {"text_uz": "Yer Quyosh atrofida aylanish?", "option_a": "24 soat", "option_b": "30 kun", "option_c": "365 kun 6 soat", "option_d": "100 yil", "correct_answer": "C"},
        {"text_uz": "Eng katta maydonga ega davlat?", "option_a": "Xitoy", "option_b": "AQSH", "option_c": "Kanada", "option_d": "Rossiya", "correct_answer": "D"},
        ...
        {"text_uz": "O'zbekiston iqlim zonasi?", "option_a": "Mo''tadil", "option_b": "Subtropik", "option_c": "Kontinental (quruq)", "option_d": "Arktik", "correct_answer": "C"},
        {"text_uz": "Sirdaryo qayerga quyiladi?", "option_a": "Kaspiy", "option_b": "Orol", "option_c": "Qora dengiz", "option_d": "Hind okeani", "correct_answer": "B"},
        {"text_uz": "O'zbekistonning eng uzun chegarasi?", "option_a": "Tojikiston", "option_b": "Qozog'iston", "option_c": "Afg'oniston", "option_d": "Turkmaniston", "correct_answer": "B"},
        {"text_uz": "Geografik kenglik nima?", "option_a": "Ekvatordan janub-shimol masofa", "option_b": "Grinvichdan sharq-g'arb masofa", "option_c": "Dengizdan balandlik", "option_d": "Ikki nuqta masofa", "correct_answer": "A"},
        {"text_uz": "Geografik uzunlik nima?", "option_a": "Ekvatordan masofa", "option_b": "Grinvich meridianidan masofa", "option_c": "Balandlik", "option_d": "Chuqurlik", "correct_answer": "B"},
        {"text_uz": "Farg'ona vodiysi qaysi tog'lar?", "option_a": "Tyanshan va Pomir", "option_b": "Qoraqum va Qizilqum", "option_c": "Oltoy va Ural", "option_d": "Kavkaz va Himoloy", "correct_answer": "A"},
        ...
        {"text_uz": "Qizilqum cho'li qayerda?", "option_a": "Faqat Turkmanistonda", "option_b": "O'zbekiston va Qozog'iston", "option_c": "Faqat O'zbekistonda", "option_d": "Tojikiston va O'zbekistonda", "correct_answer": "B"},
        {"text_uz": "Zarafshon qayerdan boshlanadi?", "option_a": "Toshkent viloyatida", "option_b": "Qirg'izistonda", "option_c": "Tojikistonda", "option_d": "Afg'onistonda", "correct_answer": "C"},
        {"text_uz": "Ekvator qayerda?", "option_a": "Shimoliy qutbda", "option_b": "Janubiy qutbda", "option_c": "Yerning o'rta qismida", "option_d": "Tropiklar bo'ylab", "correct_answer": "C"},
        ...
        {"text_uz": "O'rta dengiz qit'alar orasida?", "option_a": "Osiyo va Amerika", "option_b": "Yevropa, Afrika va Osiyo", "option_c": "Afrika va Avstraliya", "option_d": "Yevropa va Amerika", "correct_answer": "B"},
        {"text_uz": "Toshkent dengiz sathidan balandligi?", "option_a": "200-300 m", "option_b": "400-500 m", "option_c": "600-700 m", "option_d": "800-1000 m", "correct_answer": "B"},
        {"text_uz": "Eng uzun tog' tizmasi?", "option_a": "Himoloy", "option_b": "And tog'lari", "option_c": "Alp", "option_d": "Kavkaz", "correct_answer": "B"},
        {"text_uz": "O'zbekistondan qaysi daryo o'tadi?", "option_a": "Volga", "option_b": "Amudaryo", "option_c": "Don", "option_d": "Kama", "correct_answer": "B"},
    ],
    9: [  # Ingliz tili
        {"text_uz": "'Kitob' inglizcha?", "option_a": "Book", "option_b": "Pen", "option_c": "Table", "option_d": "Chair", "correct_answer": "A"},
        ...
        {"text_uz": "Present Continuous: She ___ now. (study)", "option_a": "study", "option_b": "studies", "option_c": "is studying", "option_d": "studied", "correct_answer": "C"},
        ...
        {"text_uz": "Present Perfect: She ___ book. (read)", "option_a": "read", "option_b": "has read", "option_c": "have read", "option_d": "reads", "correct_answer": "B"},
        ...
        {"text_uz": "Preposition: interested ___ music.", "option_a": "at", "option_b": "on", "option_c": "in", "option_d": "of", "correct_answer": "C"},
        {"text_uz": "Modal obligation: You ___ wear seatbelt.", "option_a": "can", "option_b": "must", "option_c": "may", "option_d": "might", "correct_answer": "B"},
        {"text_uz": "Conditional: If it rains, I ___ stay.", "option_a": "will", "option_b": "would", "option_c": "should", "option_d": "shall", "correct_answer": "A"},
        {"text_uz": "Indirect: He said he ___ tired.", "option_a": "is", "option_b": "was", "option_c": "are", "option_d": "were", "correct_answer": "B"},
    ...
    10: [  # Rus tili
        {"text_uz": "'Kitob' ruscha?", "option_a": "Книга", "option_b": "Ручка", "option_c": "Стол", "option_d": "Стул", "correct_answer": "A"},
        ...
        {"text_uz": "Я ___ (читать) — hozirgi zamon.", "option_a": "читает", "option_b": "читаешь", "option_c": "читаю", "option_d": "читают", "correct_answer": "C"},
        {"text_uz": "Он ___ (читать) — hozirgi zamon.", "option_a": "читаю", "option_b": "читает", "option_c": "читаешь", "option_d": "читаем", "correct_answer": "B"},
        {"text_uz": "'Большой' antonimi?", "option_a": "Огромный", "option_b": "Средний", "option_c": "Маленький", "option_d": "Широкий", "correct_answer": "C"},
        {"text_uz": "Он ___ (идти) — o'tgan zamon.", "option_a": "идёт", "option_b": "шёл", "option_c": "пойдёт", "option_d": "ходит", "correct_answer": "B"},
        {"text_uz": "Я ___ (читать) — kelasi zamon.", "option_a": "читаю", "option_b": "читал", "option_c": "буду читать", "option_d": "читает", "correct_answer": "C"},
        ...
        {"text_uz": "Совершенный вид nimani bildiradi?", "option_a": "Tugallanmagan", "option_b": "Tugallangan", "option_c": "Doimiy", "option_d": "Takroriy", "correct_answer": "B"},
        {"text_uz": "Несовершенный вид nimani bildiradi?", "option_a": "Tugallangan", "option_b": "Tugallanmagan", "option_c": "Bir marta", "option_d": "O'tgan", "correct_answer": "B"},
        ...
        {"text_uz": "Я иду ___ магазина (chiqish).", "option_a": "в", "option_b": "к", "option_c": "из", "option_d": "на", "correct_answer": "C"},
        ...
        {"text_uz": "Мне нравится музыка — qaysi kelishik?", "option_a": "Именительный", "option_b": "Родительный", "option_c": "Дательный", "option_d": "Винительный", "correct_answer": "A"},
        {"text_uz": "'Хотеть' fe'li qaysi spryajenie?", "option_a": "I", "option_b": "II", "option_c": "Raznosprягaemoe", "option_d": "Irregular", "correct_answer": "C"},
    ],
}
...
def cmd_seed():
    ...
    try:
        ...
        if existing >= 300:
            print(f"Savollar allaqachon bor: {existing} ta. O'tkazish uchun '--force' qo'shing.")
        ...
        for subject_id, questions in QUESTIONS_DATA.items():
            ...
            if not subject:
                print(f"❌ Fan {subject_id} topilmadi — avval init qiling")
                continue
            for q_data in questions:
                db.add(Question(
                    subject_id=subject_id,
                    text_uz=q_data["text_uz"],
                    text_oz=q_data["text_uz"],
                    text_ru=q_data["text_uz"],
...
def cmd_status():
    """Baza holatini ko'rish."""
    from database.db import Session
    from database.models import User, Question, Direction, Region, Score, Leaderboard

    db = Session()
    try:
        print("\n📊 Baza holati:")
        print(f"  👤 Foydalanuvchilar: {db.query(User).count()}")
        print(f"  ❓ Savollar:         {db.query(Question).count()}")
        print(f"  📚 Yo'nalishlar:     {db.query(Direction).count()}")
        print(f"  🗺  Viloyatlar:       {db.query(Region).count()}")
        print(f"  📈 Scorlar (jami):   {db.query(Score).count()}")
        from database.models import Score as S
        archived = db.query(S).filter(S.is_archived == True).count()
        active_s = db.query(S).filter(S.is_archived == False).count()
        print(f"     ├ Joriy:          {active_s}")
        print(f"     └ Arxivlangan:    {archived}")
        print(f"  🏆 Leaderboard:      {db.query(Leaderboard).count()}")
    except Exception as e:
        print(f"❌ Baza ulanmadi: {e}")
    finally:
        db.close()
...
COMMANDS = {
    "init":    (cmd_init,    "Jadvallar + asosiy seed"),
    "reset":   (cmd_reset,   "Bazani o'chirib qayta yaratish"),
    "seed":    (cmd_seed,    "Savollarni qo'shish"),
    "migrate": (cmd_migrate, "Mavjud bazaga yangi ustunlar"),
    "status":  (cmd_status,  "Baza holatini ko'rish"),
}
